...
export const AskTable = ({ asks }: { asks: [string, string][] }) => {
    let currentTotal = 0;
    let relevantAsks: [string, string][] = [];
    let index = 0;

    while (relevantAsks.length < 15 && index < asks.length) {
        const chunk = asks.slice(index, index + 15).filter(([price, quantity]) => Number(quantity) !== 0.00);
        relevantAsks = relevantAsks.concat(chunk);
        index += 15;
    }

    relevantAsks = relevantAsks.slice(0, 15);
