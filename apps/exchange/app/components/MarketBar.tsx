...
import { Ticker as LocalTicker } from "../utils/types";
import { getTicker } from "../utils/httpClient";
import { SignalingManager } from "../utils/SignalingManager";

type MarketBarProps = {
    market: string;
    LastPrice: (price: any) => void;
};

export const MarketBar = ({ market, LastPrice }: MarketBarProps) => {
    const [ticker, setTicker] = useState<LocalTicker | null>(null);


    useEffect(() => {
        getTicker(market).then(setTicker);
        SignalingManager.getInstance().registerCallback("ticker", (data: Partial<LocalTicker>) => setTicker(prevTicker => ({
